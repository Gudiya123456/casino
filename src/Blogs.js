...
const Blogs = () => {
  ...
  const blogData = async () => {
    try {
      const response = await axios.get('http://casinosonlinein.com/1xbet/console/api/blogs')
                ...
                <div className=' col-span-12 md:col-span-6 lg:col-span-4 rounded-lg shadow-md'>
                  <div className=' bg-light'>
                    <div key={blog.id}>
                      ...
                      <Link to={`/blogs/${blog.id}`}>
                          <img src={`http://casinosonlinein.com/1xbet/console/public/storage/${blog.image}`} alt="" className=' object-streach w-full h-[260px]' />
