...
const Header = ({ settings }) => {
  ...
  const bannerdata = async () => {
    try {
      const response = await axios.get('http://casinosonlinein.com/1xbet/console/api/banners')
  ...
  const backgroundStyle = {
    ...
    backgroundImage: `linear-gradient(rgba(0, 0, 0, 0) 0%, rgba(0, 0, 0, 0.9) 100%), url(${`http://casinosonlinein.com//1xbet/console/public/storage/${settings?.background}`})`,
  };
        ...
        <div className="flex items-strech flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-6 lg:space-x-8">
          <div style={backgroundStyle} className="flex flex-col md:flex-row items-strech justify-between w-full py-6 px-6 md:py-12 lg:px-12">
            <div className="flex flex-col justify-center md:w-1/2">
              <div className="text-3xl lg:text-4xl font-semibold text-gray-800 dark:text-white"><img src={`http://casinosonlinein.com/1xbet/console/public/storage/${settings?.logo}`} alt="" className="" /></div>
            </div>
            ...
            <Slider {...carousalsettings} arrows={false} className='flex mt-8 md:mt-0 mb-10 items-center md:justify-end md:me-20 md:w-[500px] h-[119px] lg:self-center'>
              ...
              {banners?.map((banner, index) => (
                ...
                <img src={`http://casinosonlinein.com/1xbet/console/public/storage/${banner.banner_image}`} alt={banner.banner_name} className="w-[500px] h-[119px] object-cover" />
